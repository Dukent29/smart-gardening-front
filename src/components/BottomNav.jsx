          ...
          <Link key={href} href={href} className="flex flex-col items-center group">
            <span
              className={`text-xl rounded-full p-2 transition ${
                isActive ? "bg-[#0a5d2f] text-white" : "text-[#0a5d2f] group-hover:bg-[#0a5d2f] group-hover:text-white"
              }`}
            >
              {icon}
            </span>
            <span className={`text-xs mt-1 ${isActive ? "text-[#0a5d2f] font-semibold" : "text-gray-700"}`}>
              {label}
            </span>
          </Link>
        );
